...
[Browser]
    ↓ HTTPS (Port 443)
[Traefik Proxy]  ← SSL termination (Let's Encrypt), Ruteo dinámico
    ↓
┌───────────────────────┐      ┌────────────────────────┐
│  Frontend (Next.js)   │      │   API Gateway (Spring) │
│  app.midominio.com    │      │   /api/...             │
└───────────────────────┘      └────────────────────────┘
    ↓                                  ↓
    └──────────────────┬───────────────┘
...
- No microservice (including Gateway and Frontend) exposes ports to the host in production.
- Only Traefik (Proxy) exposes ports 80 (HTTP) and 443 (HTTPS).
- All traffic is encrypted via SSL (Auto-generated by Let's Encrypt).
