...
DEFAULT_LOCAL_SERVICES="service-users service-finances service-investments gateway"
...
cmd_local_all() {
    ...
    echo ""
    echo "  Stop with:   ./dev.sh stop-all"
    echo "  Tail logs:   ./dev.sh logs-local [service]"
    echo "  Check PIDs:  ./dev.sh status-local"
}
...
cmd_help() {
    ...
    echo -e "  ${CYAN}./dev.sh local-all${RESET}                            Infra + default services in background"
    echo -e "  ${CYAN}./dev.sh local-all service-users gateway${RESET}      Infra + selected services in background"
    echo -e "  ${CYAN}./dev.sh local-all --front${RESET}                    Add frontend to the mix"
    echo -e "  ${CYAN}./dev.sh stop-all${RESET}                             Kill all local background processes"
    echo -e "  ${CYAN}./dev.sh logs-local${RESET}                           Tail all local log files"
    echo -e "  ${CYAN}./dev.sh logs-local service-finances${RESET}          Tail a specific log file"
    echo -e "  ${CYAN}./dev.sh status-local${RESET}                         Show which local processes are alive"
    echo ""
    echo -e "  ${BOLD}── Local (single service, foreground) ────────────────────────────────${RESET}"
    echo -e "  ${CYAN}./dev.sh local service-finances${RESET}               Infra + run one service with Maven"
    echo -e "  ${CYAN}./dev.sh front${RESET}                                Infra + run frontend with npm"
    echo ""
    echo -e "  ${BOLD}── Docker ────────────────────────────────────────────────────────────${RESET}"
    echo -e "  ${CYAN}./dev.sh infra${RESET}                                Start infrastructure only"
    echo -e "  ${CYAN}./dev.sh dev service-finances${RESET}                 Infra + build + run one service in Docker"
    echo -e "  ${CYAN}./dev.sh up${RESET}                                   Build + start ALL services (ports exposed)"
    echo -e "  ${CYAN}./dev.sh prod${RESET}                                 Build + start ALL services (ports hidden)"
    echo -e "  ${CYAN}./dev.sh down${RESET}                                 Stop and remove all containers"
    echo -e "  ${CYAN}./dev.sh build [svc]${RESET}                          Rebuild all or specific image"
    echo -e "  ${CYAN}./dev.sh restart [svc]${RESET}                        Restart all or specific container"
    echo -e "  ${CYAN}./dev.sh logs [svc]${RESET}                           Follow Docker logs"
